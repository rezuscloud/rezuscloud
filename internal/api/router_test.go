...
func setupRouter(t *testing.T) (*state.Store, http.Handler, *auth.JWTManager) {
	...
	return store, Router(store, jwtManager, audit.NewComponent(store.DB(), audit.ComponentOptions{}), nil, upgrade.NewManager(store, upgrade.NoOpMachineUpgrader{}, upgrade.NewStoreMachineLister(store)), nil, nil, nil, nil), jwtManager
}
