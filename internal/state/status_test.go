...
func TestComputeTenantPhase_NoMachines(t *testing.T) {
	tenant := &Tenant{
		...
		Spec:     TenantSpec{KubernetesVersion: "1.35.0"},
	}

	phase := ComputeTenantPhase(tenant, nil, nil)
	if phase != TenantForming {
		t.Errorf("phase = %q, want %q", phase, TenantForming)
	}
}
...
func TestComputeTenantPhase_Removing(t *testing.T) {
	ts := time.Now().UTC()
	tenant := &Tenant{
		Metadata: Metadata{Name: "test", DeletionTimestamp: &ts},
	}

	phase := ComputeTenantPhase(tenant, nil, nil)
	if phase != TenantRemoving {
		t.Errorf("phase = %q, want %q", phase, TenantRemoving)
	}
}
...
func TestComputeTenantPhase_Active(t *testing.T) {
	...
	machines := []*Machine{
		{Metadata: Metadata{Name: "m1", Labels: map[string]string{"rezuscloud.io/role": "controlplane"}}, Status: MachineStatus{Ready: true, Stage: StageReady}},
		{Metadata: Metadata{Name: "m2", Labels: map[string]string{"rezuscloud.io/role": "worker"}}, Status: MachineStatus{Ready: true, Stage: StageReady}},
	}

	nodeGroups := []NodeGroupSummary{{Name: "cp", Count: 1}, {Name: "workers", Count: 1}}

	phase := ComputeTenantPhase(tenant, machines, nodeGroups)
	if phase != TenantActive {
		t.Errorf("phase = %q, want %q", phase, TenantActive)
	}
}
...
func TestComputeTenantPhase_Forming_NotEnoughMachines(t *testing.T) {
	...
	machines := []*Machine{
		{Metadata: Metadata{Name: "m1"}, Status: MachineStatus{Ready: true, Stage: StageReady}},
	}
...
func TestComputeTenantPhase_Forming_NotAllReady(t *testing.T) {
	...
	machines := []*Machine{
		{Metadata: Metadata{Name: "m1"}, Status: MachineStatus{Ready: true, Stage: StageReady}},
		{Metadata: Metadata{Name: "m2"}, Status: MachineStatus{Ready: false, Stage: StageInstalling}},
	}
...
func TestComputeTenantPhase_Shrinking(t *testing.T) {
	...
	machines := []*Machine{
		{Metadata: Metadata{Name: "m1"}, Status: MachineStatus{Ready: true, Stage: StageReady}},
		{Metadata: Metadata{Name: "m2"}, Status: MachineStatus{Ready: true, Stage: StageReady}},
		{Metadata: Metadata{Name: "m3"}, Status: MachineStatus{Ready: true, Stage: StageReady}},
	}

	nodeGroups := []NodeGroupSummary{{Name: "cp", Count: 1}, {Name: "workers", Count: 1}}

	phase := ComputeTenantPhase(tenant, machines, nodeGroups)
	if phase != TenantShrinking {
		t.Errorf("phase = %q, want %q", phase, TenantShrinking)
	}
}
...
func TestComputeTenantStatus_FullStatus(t *testing.T) {
	tenant := &Tenant{
		Metadata: Metadata{Name: "test"},
		Spec:     TenantSpec{KubernetesVersion: "1.35.0", TalosVersion: "1.12.6"},
	}

	machines := []*Machine{
		{Metadata: Metadata{Name: "m1", Labels: map[string]string{"rezuscloud.io/role": "controlplane"}}, Spec: MachineSpec{Connected: true}, Status: MachineStatus{Ready: true, Stage: StageReady}},
		{Metadata: Metadata{Name: "m2", Labels: map[string]string{"rezuscloud.io/role": "worker"}}, Spec: MachineSpec{Connected: true}, Status: MachineStatus{Ready: true, Stage: StageReady}},
	}
	...
	if status.Phase != TenantActive {
		t.Errorf("phase = %q, want %q", status.Phase, TenantActive)
	}
	if !status.Available {
...
func TestComputeTenantStatus_Forming_WithControlPlane(t *testing.T) {
	...
	machines := []*Machine{
		{Metadata: Metadata{Name: "m1", Labels: map[string]string{"rezuscloud.io/role": "controlplane"}}, Status: MachineStatus{Ready: true, Stage: StageReady}},
		{Metadata: Metadata{Name: "m2", Labels: map[string]string{"rezuscloud.io/role": "worker"}}, Status: MachineStatus{Ready: false, Stage: StageInstalling}},
	}
	...
	if status.Phase != TenantForming {
		t.Errorf("phase = %q, want %q", status.Phase, TenantForming)
	}
...
func TestComputeTenantPhase_Transitions(t *testing.T) {
	...
	machines := []*Machine{
		{Status: MachineStatus{Ready: false, Stage: StageInstalling}},
	}
	phase = ComputeTenantPhase(tenant, machines, nodeGroups)
	if phase != TenantForming {
	...
	machines[0].Status = MachineStatus{Ready: true, Stage: StageReady}
	phase = ComputeTenantPhase(tenant, machines, nodeGroups)
	if phase != TenantActive {
