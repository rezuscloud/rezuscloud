...
func cliTestEnv(t *testing.T) (*apiclient.Client, *state.Store, string) {
	...
	handler := api.Router(store, jwtManager, audit.NewComponent(store.DB(), audit.ComponentOptions{}), nil, upgrade.NewManager(store, upgrade.NoOpMachineUpgrader{}, upgrade.NewStoreMachineLister(store)), nil, nil, nil, nil)
