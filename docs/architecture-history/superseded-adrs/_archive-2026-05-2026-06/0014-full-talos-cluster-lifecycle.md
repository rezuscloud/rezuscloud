...
> **Note on config delivery**: This ADR originally referenced SideroLink as the config push transport. SideroLink is rejected ([ADR 13](0013-siderolink-config-pull.md)). Config is delivered via `user_data` (cloud VMs) and the Talos API / `talos_machine_configuration_apply` (bare metal), generated by the TF Talos provider during `tofu apply`. The lifecycle phases below still apply; only the transport changed.
